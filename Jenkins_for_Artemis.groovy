node {
	...
	name: 'ENVIR'),
	string(defaultValue: 'kumarova.t@gmail.com', description: 'please enter an email to receive Artemis deploy', name: 'EMAIL_TO_SEND', trim: false)
	])])
	stage("Stage1"){
		timestamps {
			ws {
	...
	stage ("send email")
	mail bcc: '', body: "Hello! Artemis ${Version} deployed to ${Envir}" , cc: '', from: '', replyTo: '', subject: "Artemis version deployed to environment", to: "${EMAIL_TO_SEND}"
	}
}